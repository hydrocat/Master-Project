...
When developing a enterprise-level application, often times there is a need for some sort of persistence storage, in pactice this usually translates to a \gls{RDBMS}. To reduce the amount of code needed to manage such interactions, Spring Data module was developed. Its main interface is called \texttt{Repository} and it decouples the entities that are being persisted from the underlying storage system~\cite{springdata}.

Although it acts as the root interface through which other, more feature packed, models are developed, \texttt{Repository} is hardly used directly as it's a marker for repositories~\ref{springdatadoc}, instead \texttt{CrudRepository} and \texttt{PagingAngSortingRepository} are interfaces that provide more tangible features.

\texttt{CrudRepository} offers, as the name suggests, \gls{CRUD} capabilities for a specified entity and \texttt{PagingAngSortingRepository} extends it by including, as the name suggests, paging and sorting support.

The \texttt{CrudRepository} on its own provide the following default methods:
\todo{o que faz, quais sao os paremetros}
\begin{itemize}
\item \textit{save(Entity)}
\item \textit{findOne(ID)}
\item \textit{findAll()}
\item \textit{count()}
\item \textit{delete(Entity)}
\item \textit{exists(ID)}
\end{itemize}

But customized methods may be added by inserting a properly named method definition to the extended interface.

With this, implementing simple \gls{CRUD} operations is a quick process:
\begin{enumerate}
\item Create a new interface that extends the desired repository, e.g.e \texttt{CrudRepository}.
\item Add method definitions that fits the business-specific requirements.
\item Anywhere else in the application, declare a dependency on the new interface.
\item Use the provided methods to retrieve the persisted objects.
\end{enumerate}
